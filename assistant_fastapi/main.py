...
from .assistant.rag_assistant import process, save_to_vector_storage, delete_from_vector_storage
import httpx
import json
...
async def get_answer(request: Request):
    try:
        ...
        if response.status_code == 200:
            answer = await process(data['query'], django_data['id'])
...
async def save_vector(request: Request):
    try:
        data = await request.json()
        await save_to_vector_storage(data['note'])
...
async def delete_vector(request: Request):
    try:
        data = await request.json()
        await delete_from_vector_storage(ids=[data['note_id']])
